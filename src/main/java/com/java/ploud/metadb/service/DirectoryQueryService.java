...
import com.java.ploud.exceptions.DirectoryNotFoundException;
...
public class DirectoryQueryService {
     ...
     */
    @Transactional(readOnly = true)
    public List<Files> findAllFilesByDirectories(List<Directory> directories) {
        try {
            List<Long> dirSeqs = directories.stream()
                    .map(Directory::getDirSeq)
                    .toList();
            List<Files> files = fileRepository.findAllByParent_DirSeqIn(dirSeqs);
